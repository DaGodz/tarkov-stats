exports.handler = async (event, context, callback) => {
    ...
    var match = raidOutcomeMatch.exec(extractedText);
    ...
    if (playerType == "PMC") {
        ...
        var match = currentLevelMatch.exec(extractedText);
        var currentLevel = match[2]
    }
    
    if (raidOutcome == "KIA") {
        var deathLocationMatch = /(\((\w+,* *)*\))/g;
        var match = deathLocationMatch.exec(extractedText);
        var deathLocation = match[1].substring(1,match[1].length-1)
    }
    
    var durationMatch = /(\d{2}:\d{2}:\d{2})/g;
    var match = durationMatch.exec(extractedText);
    ...
    var duration = String((hours * 3600) + (minutes * 60) + seconds);
    
    var xpMatch = /(EXP )([0-9, ]*)/g;
    var match = xpMatch.exec(extractedText);
